...
function getService(params, callback) {
  ...
  var protocol =
    this.options.Protocol || (util.isBrowser && location.protocol === 'http:' ? 'http:' : 'https:')
...
function putBucketCors(params, callback) {
  ...
  util.each(CORSRules, function(rule) {
    util.each(['AllowedOrigin', 'AllowedHeader', 'AllowedMethod', 'ExposeHeader'], function(
      key,
      k
    ) {
      var sKey = key + 's'
    ...
    function(err, data) {
      if (err) {
        if (
          err.statusCode === 404 &&
          err.error &&
          err.error.Code === 'NoSuchLifecycleConfiguration'
        ) {
          var result = {
            Rules: [],
            statusCode: err.statusCode
...
function putBucketWebsite(params, callback) {
  ...
  var RoutingRules =
    WebsiteConfiguration['RoutingRules'] || WebsiteConfiguration['RoutingRule'] || []
...
function getObjectUrl(params, callback) {
  ...
  var AuthData = getAuthorizationAsync.call(
    ...
    {
      Action:
        (params.Method || '').toUpperCase() === 'PUT' ? 'name/cos:PutObject' : 'name/cos:GetObject',
      Bucket: params.Bucket || '',
      Region: params.Region || '',
    ...
    function(err, AuthData) {
      ...
      AuthData.XCosSecurityToken &&
        (signUrl += '&x-cos-security-token=' + AuthData.XCosSecurityToken)
...
function getUrl(params) {
  ...
  var protocol =
    params.protocol || (util.isBrowser && location.protocol === 'http:' ? 'http:' : 'https:')
  if (!domain) {
    if (
      ['cn-south', 'cn-south-2', 'cn-north', 'cn-east', 'cn-southwest', 'sg'].indexOf(region) > -1
    ) {
...
function getAuthorizationAsync(params, callback) {
  ...
  ;(function() {
    ...
    for (i = self._StsCache.length - 1; i >= 0; i--) {
      ...
      if (
        (AuthData.StartTime && compareTime < AuthData.StartTime) ||
        compareTime >= AuthData.ExpiredTime
      ) {
  ...
  var calcAuthByTmpKey = function() {
    var KeyTime =
      StsData.StartTime && StsData.ExpiredTime ? StsData.StartTime + ';' + StsData.ExpiredTime : ''
...
function _submitRequest(params, callback) {
  ...
  if (TaskId && !self._isRunningTask(TaskId)) {
    return
  }
  ...
  params.AuthData.XCosSecurityToken &&
    (opt.headers['x-cos-security-token'] = params.AuthData.XCosSecurityToken)
  ...
  sender.on('response', function(response) {
    ...
    if (statusSuccess && params.outputStream) {
      ...
      responseContentLength >= process.binding('buffer').kMaxLength &&
      opt.method !== 'HEAD'
    ) {
      cb({
        error:
