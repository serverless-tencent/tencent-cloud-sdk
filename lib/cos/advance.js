...
function sliceUploadFile(params, callback) {
  ...
  ;(function() {
    ...
    for (var i = 0; i < SIZE.length; i++) {
      ...
      if (FileSize / AutoChunkSize <= self.options.MaxPartNumber) {
        break
      }
    }
...
function uploadSliceList(params, cb) {
  ...
  var needUploadSlices = util.filter(UploadData.PartList, function(SliceItem) {
    if (SliceItem['Uploaded']) {
      FinishSize +=
        SliceItem['PartNumber'] >= SliceCount ? FileSize % SliceSize || SliceSize : SliceSize
    }
  ...
  Async.eachLimit(
    ...
    function(SliceItem, asyncCallback) {
      ...
      var currentSize =
        Math.min(FileSize, SliceItem['PartNumber'] * SliceSize) -
        (SliceItem['PartNumber'] - 1) * SliceSize
      var preAddSize = 0
...
function abortUploadTask(params, callback) {
  ...
  if (Level === 'bucket') {
    ...
    if (!Key) {
      return callback({ error: 'abort_upload_task_no_key' })
    }
    wholeMultipartList.call(
      self,
      {
    ...
    if (!UploadId) {
      return callback({ error: 'abort_upload_task_no_id' })
    }
    if (!Key) {
      return callback({ error: 'abort_upload_task_no_key' })
    }
...
function sliceCopyFile(params, callback) {
  ...
  var CopySliceSize =
    params.CopySliceSize === undefined ? self.options.CopySliceSize : params.CopySliceSize
